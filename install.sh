...
SPRITE_DIR=~/web2py/applications/solver/static/images/sprites
SHIP_DIR=~/web2py/applications/solver/static/images/ships
SHEETS_DIR=~/web2py/applications/solver/static/images/sprite_sheets

mkdir -p ${SPRITE_DIR} ${SHIP_DIR} ${SHEETS_DIR}
...
for SPRITE in ${CUSTOM_SPRITES}; do
    [ -L ${SPRITE_DIR}/${SPRITE}.png ] || ln -s ${CUR}/samus_sprites/${SPRITE}.png ${SPRITE_DIR}/${SPRITE}.png
    [ -L ${SHEETS_DIR}/${SPRITE}.png ] || ln -s ${CUR}/sprite_sheets/${SPRITE}.png ${SHEETS_DIR}/${SPRITE}.png
done

for SHIP in ${CUSTOM_SHIPS}; do
    [ -L ${SHIP_DIR}/${SHIP}.png ] || ln -s ${CUR}/ship_sprites/${SHIP}.png ${SHIP_DIR}/${SHIP}.png
done
